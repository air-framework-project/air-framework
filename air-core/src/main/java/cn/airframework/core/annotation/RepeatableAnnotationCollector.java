...
public interface RepeatableAnnotationCollector {
	 ...
	 * @return {@code RepeatableAnnotationCollector}实例
	 */
	static RepeatableAnnotationCollector any() {
		return Any.INSTANCE;
	}
	 ...
	 * @return {@code RepeatableAnnotationCollector}实例
	 */
	static RepeatableAnnotationCollector custom(final BiPredicate<Annotation, Method> predicate) {
	...
	static void clearSingletonCaches() {
		...
		Any.INSTANCE.repeatableMethodCache.clear();
	}
	...
	abstract class AbstractCollector implements RepeatableAnnotationCollector {
		...
		private List<Annotation> find(
			final Annotation annotation, final Predicate<Annotation> condition, final boolean accumulate) {
			if (Objects.isNull(annotation)) {
				return Collections.emptyList();
			}
			...
			while (!deque.isEmpty()) {
				...
				if (CollectionUtils.isEmpty(repeatableMethods) || isTarget) {
				...
				if (ArrayUtils.isNotEmpty(repeatableAnnotation)) {
					CollectionUtils.addAll(deque, repeatableAnnotation);
				}
			}
			return results;
		}
	...
	class Standard extends AbstractCollector {
		 ...
		 * 可重复注解对应的方法缓存
		 */
		private final Map<Class<? extends Annotation>, Object> repeatableMethodCache = new ConcurrentHashMap<>();
		...
		private Object resolveRepeatableMethodFromType(final Class<? extends Annotation> annotationType) {
			final Method[] attributes = AnnotationUtils.getAnnotationAttributes(annotationType);
		...
		protected boolean isRepeatableMethod(final Method attribute) {
			// 属性名需为“value”
			if (!Objects.equals(VALUE, attribute.getName())) {
				return false;
			}
	...
	class Any extends AbstractCollector {
		 ...
		 */
		private Object resolveRepeatableMethodFromType(final Class<? extends Annotation> annotationType) {
			final List<Method> methods = Stream.of(AnnotationUtils.getAnnotationAttributes(annotationType))
				.filter(Any::isRepeatableMethod)
				.toList();
		 ...
		 * @return 该属性是否为注解存放可重复注解的方法
		 */
		private static boolean isRepeatableMethod(final Method attribute) {
			final Class<?> attributeType = attribute.getReturnType();
			// 返回值类型需为数组
			return attributeType.isArray()
				// 数组元素需为注解
	...
	class Custom extends AbstractCollector {
		...
		@Override
		protected List<Method> resolveRepeatableMethod(final Annotation annotation) {
			return Stream.of(AnnotationUtils.getAnnotationAttributes(annotation.annotationType()))
